...
EMFLAGS=-s USE_SDL=2 -s USE_SDL_MIXER=2 -s USE_SDL_IMAGE=2 -s USE_SDL_TTF=2 -s SDL2_IMAGE_FORMATS='["png"]' -s USE_OGG=1 -s USE_VORBIS=1 -s ALLOW_MEMORY_GROWTH=1 -fno-rtti -fno-exceptions
EXTRA=
...
ifeq ($(UNAME), Windows)
	...
	LDLIBS=`$(PKG_CONFIG) sdl2 SDL2_mixer SDL2_image SDL2_ttf --cflags --libs` -Wl,-Bstatic
...
ifeq ($(UNAME), Darwin)
	...
	LDLIBS=-framework SDL2 -framework SDL2_mixer -framework SDL2_image -framework SDL2_ttf -rpath @executable_path/../Frameworks -F /Library/Frameworks/
